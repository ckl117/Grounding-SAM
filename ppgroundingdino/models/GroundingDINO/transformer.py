...
from .initializer import constant_, xavier_uniform_, normal_
...
class Transformer(nn.Layer):
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, text_dict=None):
        ...
        if self.two_stage_type == "standard":
            ...
            # topk_proposals = paddle.to_tensor(np.load('th_topk.npy'))

            # batch_ind = paddle.arange(end=topk_proposals.shape[0], dtype=topk_proposals.dtype)
            # batch_ind = batch_ind.unsqueeze(-1).tile([1, self.num_queries])
            # topk_ind = paddle.stack([batch_ind, topk_proposals], axis=-1)
            ...
            # gather boxes
            # refpoint_embed_undetach = paddle.gather_nd(enc_outputs_coord_unselected, topk_ind)  # unsigmoid
            # refpoint_embed_ = refpoint_embed_undetach.detach()
            # init_box_proposal = F.sigmoid(paddle.gather_nd(output_proposals, topk_ind)) # sigmoid
