...
class SwinTransformerBlock(nn.Layer):
    ...
    def forward(self, x, mask_matrix):
        ...
        pad_list = paddle.to_tensor([pad_l, pad_r, pad_t,pad_b],dtype="int32")
...
class PatchMerging(nn.Layer):
    ...
    def forward(self, x, H, W):
        ...
        if pad_input:
            pad_list = paddle.to_tensor([0, H % 2,0,W % 2],dtype="int32")
            x = F.pad(x, pad_list)
...
class PatchEmbed(nn.Layer):
    ...
    def forward(self, x):
        ...
        if W % self.patch_size[1] != 0:
            pad_list = paddle.to_tensor([0, self.patch_size[1] - W % self.patch_size[1], 0, 0],dtype="int32")
            x = F.pad(x, pad_list)
        if H % self.patch_size[0] != 0:
            pad_list = paddle.to_tensor([0, 0, 0, self.patch_size[0] - H % self.patch_size[0]],dtype="int32")
            x = F.pad(x, pad_list)
...
class SwinTransformer(nn.Layer):
    ...
    def forward(self, x:paddle.Tensor, m:paddle.Tensor):
...
if __name__ == "__main__":
    ...
    x = paddle.rand([2, 3, 1024, 1024])
    y = model.forward_raw(x)
    import ipdb

    ipdb.set_trace()
    x = paddle.rand([2, 3, 384, 384])
    y = model.forward_raw(x)
