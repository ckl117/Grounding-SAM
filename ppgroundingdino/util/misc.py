...
class NestedTensor(object):
    ...
    def __init__(self, tensors, mask: Optional[paddle.Tensor]):
        ...
        if isinstance(mask, str) and mask == "auto":
            self.mask = paddle.zeros_like(tensors).astype(tensors.dtype)
            if self.mask.dim() == 3:
                self.mask = self.mask.sum(0).astype(paddle.bool)
            elif self.mask.dim() == 4:
                self.mask = self.mask.sum(1).astype(bool)
            else:
                raise ValueError(
    ...
    def imgsize(self):
        ...
        for i in range(self.tensors.shape[0]):
            ...
            maxH = (~mask).sum(0).max()
            maxW = (~mask).sum(1).max()
            res.append(paddle.to_tensor([maxH, maxW]))
        return res
